...
describe('SignUp Controller', () => {
  ...
  test('Should return 400 if password confirmation fails', async () => {
    ...
    const httpRequest = {
      body: {
        ...
        email: 'invalid_email',
        password: 'any_password',
        passwordConfirmation: 'wrong_confirmation'
      }
    }
    const httpResponse = await sut.handle(httpRequest)
    expect(httpResponse.statusCode).toBe(400)
  ...
  test('Should call emailValidor with correct email', async () => {
    ...
    expect(isValidSpy).toHaveBeenCalledWith(makeFakeRequestData().body.email)
  })

  test('Should return 500 if emailValidate throws', async () => {
    const { sut, emailValidatorStub } = makeSut()
